 ...
 */
static long xdl_split(unsigned long const *ha1, long off1, long lim1, unsigned long const *ha2,
                      long off2, long lim2, long *kvdf, long *kvdb, int need_min, xdpsplit_t *spl,
                      xdalgoenv_t *xenv)
{
    ...
    for (ec = 1;; ec++) {
        ...
        if (got_snake && ec > xenv->heur_min) {
            for (best = 0, d = fmax; d >= fmin; d -= 2) {
                ...
                if (v > XDL_K_HEUR * ec && v > best && off1 + xenv->snake_cnt <= i1 && i1 < lim1 &&
            ...
            for (best = 0, d = bmax; d >= bmin; d -= 2) {
                ...
                if (v > XDL_K_HEUR * ec && v > best && off1 < i1 && i1 <= lim1 - xenv->snake_cnt &&
 ...
 */
int xdl_recs_cmp(diffdata_t *dd1, long off1, long lim1, diffdata_t *dd2, long off2, long lim2,
...
int xdl_do_diff(mmfile_t *mf1, mmfile_t *mf2, xpparam_t const *xpp, xdfenv_t *xe)
{
    ...
    res = xdl_recs_cmp(&dd1, 0, dd1.nrec, &dd2, 0, dd2.nrec, kvdf, kvdb,
...
static void score_add_split(const struct split_measurement *m, struct split_score *s)
{
    ...
    if (indent == -1) {
        ...
        if (m->post_indent != -1 && m->post_indent > indent) {
             ...
             */
            s->penalty +=
                any_blanks ? RELATIVE_OUTDENT_WITH_BLANK_PENALTY : RELATIVE_OUTDENT_PENALTY;
        } else {
            /*
                              ...
                              xdemitconf_t const *xecfg)
{
    ...
    for (xch = xscr; xch; xch = xche->next) {
        ...
        if (xecfg->hunk_func(xch->i1, xche->i1 + xche->chg1 - xch->i1, xch->i2,
                             xche->i2 + xche->chg2 - xch->i2, ecb->priv) < 0)
            return -1;
    }
    return 0;
}
...
static int record_matches_regex(xrecord_t *rec, xpparam_t const *xpp)
{
    ...
    for (i = 0; i < xpp->ignore_regex_nr; i++)
        if (!xdl_regexec_buf(xpp->ignore_regex[i], rec->ptr, rec->size, 1, &regmatch, 0))
            return 1;

    return 0;
}

static void xdl_mark_ignorable_regex(xdchange_t *xscr, const xdfenv_t *xe, xpparam_t const *xpp)
{
    xdchange_t *xch;
...
int xdl_diff(mmfile_t *mf1, mmfile_t *mf2, xpparam_t const *xpp, xdemitconf_t const *xecfg,
             xdemitcb_t *ecb)
{
    xdchange_t *xscr;
    xdfenv_t xe;
