...
TEST_F(XDiffCliTest, MovedBlocksPlain)
{
    ...
    // Check that moved lines are marked with < and > prefix
    EXPECT_TRUE(output.find("<") != std::string::npos)
        << "Output should contain moved block markers";
    EXPECT_TRUE(output.find(">") != std::string::npos)
        << "Output should contain moved block markers";
}
...
TEST_F(XDiffCliTest, MovedBlocksWhitespaceIgnore)
{
    ...
    int status =
        runXDiffCli({ "--moved=plain", "--moved-ws=ignore-at-eol", file1.string(), file2.string() },
                    output, error);
...
TEST_F(XDiffCliTest, MovedBlocksNoChanges)
{
    ...
    EXPECT_TRUE(output.empty() || output.find("<") == std::string::npos || output.find(">") == std::string::npos)
