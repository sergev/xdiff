...
static int xdl_append_merge(xdmerge_t **merge, int mode, long i0, long chg0, long i1, long chg1,
                            long i2, long chg2)
{
    xdmerge_t *m = *merge;
...
static int xdl_merge_cmp_lines(xdfenv_t *xe1, int i1, xdfenv_t *xe2, int i2, int line_count,
                               long flags)
{
    int i;
    ...
    for (i = 0; i < line_count; i++) {
        int result = xdl_recmatch(rec1[i]->ptr, rec1[i]->size, rec2[i]->ptr, rec2[i]->size, flags);
        if (!result)
            return -1;
    }
    return 0;
...
static int xdl_recs_copy_0(int use_orig, xdfenv_t *xe, int i, int count, int needs_cr, int add_nl,
                           char *dest)
{
    xrecord_t **recs;
    int size = 0;
...
static int is_eol_crlf(xdfile_t *file, int i)
{
    ...
    if (i < file->nrec - 1)
        ...
        return (size = file->recs[i]->size) > 1 && file->recs[i]->ptr[size - 2] == '\r';
    if (!file->nrec)
    ...
    return (size = file->recs[i - 1]->size) > 1 && file->recs[i - 1]->ptr[size - 2] == '\r';
}
...
static int fill_conflict_hunk(xdfenv_t *xe1, const char *name1, xdfenv_t *xe2, const char *name2,
                              const char *name3, int size, int i, int style, xdmerge_t *m,
                              char *dest, int marker_size)
{
    ...
    size += xdl_recs_copy(xe1, m->i1, m->chg1, needs_cr, 1, dest ? dest + size : NULL);
    ...
    if (style == XDL_MERGE_DIFF3 || style == XDL_MERGE_ZEALOUS_DIFF3) {
        ...
        size += xdl_orig_copy(xe1, m->i0, m->chg0, needs_cr, 1, dest ? dest + size : NULL);
    }
    ...
    size += xdl_recs_copy(xe2, m->i2, m->chg2, needs_cr, 1, dest ? dest + size : NULL);
    if (!dest) {
...
static int xdl_fill_merge_buffer(xdfenv_t *xe1, const char *name1, xdfenv_t *xe2, const char *name2,
                                 const char *ancestor_name, int favor, xdmerge_t *m, char *dest,
                                 int style, int marker_size)
{
    int size, i;
    ...
    for (size = i = 0; m; m = m->next) {
        ...
        if (m->mode == 0)
            size = fill_conflict_hunk(xe1, name1, xe2, name2, ancestor_name, size, i, style, m,
                                      dest, marker_size);
        else if (m->mode & 3) {
            /* Before conflicting part */
            size += xdl_recs_copy(xe1, i, m->i1 - i, 0, 0, dest ? dest + size : NULL);
    ...
    size += xdl_recs_copy(xe1, i, xe1->xdf2.nrec - i, 0, 0, dest ? dest + size : NULL);
    return size;
}

static int recmatch(xrecord_t *rec1, xrecord_t *rec2, unsigned long flags)
{
    return xdl_recmatch(rec1->ptr, rec1->size, rec2->ptr, rec2->size, flags);
}
                                        ...
                                        xpparam_t const *xpp)
{
    ...
    for (; m; m = m->next) {
        ...
        while (m->chg1 && m->chg2 && recmatch(rec1[m->i1], rec2[m->i2], xpp->flags)) {
            m->chg1--;
            m->chg2--;
            m->i1++;
            m->i2++;
        ...
        while (m->chg1 && m->chg2 &&
               recmatch(rec1[m->i1 + m->chg1 - 1], rec2[m->i2 + m->chg2 - 1], xpp->flags)) {
            m->chg1--;
            m->chg2--;
        }
    }
 ...
 */
static int xdl_refine_conflicts(xdfenv_t *xe1, xdfenv_t *xe2, xdmerge_t *m, xpparam_t const *xpp)
{
    for (; m; m = m->next) {
        mmfile_t t1, t2;
        xdfenv_t xe;
 ...
 */
static int xdl_simplify_non_conflicts(xdfenv_t *xe1, xdmerge_t *m, int simplify_if_no_alnum)
{
    int result = 0;
    ...
    for (;;) {
        ...
        if (m->mode != 0 || next_m->mode != 0 ||
            (end - begin > 3 &&
             (!simplify_if_no_alnum || lines_contain_alnum(xe1, begin, end - begin)))) {
            m = next_m;
        } else {
            result++;
 ...
 */
static int xdl_do_merge(xdfenv_t *xe1, xdchange_t *xscr1, xdfenv_t *xe2, xdchange_t *xscr2,
                        xmparam_t const *xmp, mmbuffer_t *result)
{
    ...
    while (xscr1 && xscr2) {
        ...
        if (level == XDL_MERGE_MINIMAL || xscr1->i1 != xscr2->i1 || xscr1->chg1 != xscr2->chg1 ||
            xscr1->chg2 != xscr2->chg2 ||
            xdl_merge_cmp_lines(xe1, xscr1->i2, xe2, xscr2->i2, xscr1->chg2, xpp->flags)) {
            /* conflict */
            ...
            if (ffo < 0) {
                ...
                chg2 += ffo;
            if (xdl_append_merge(&c, 0, i0, chg0, i1, chg1, i2, chg2)) {
                xdl_cleanup_merge(changes);
                return -1;
            }
        }
    ...
    if (style == XDL_MERGE_ZEALOUS_DIFF3) {
        xdl_refine_zdiff3_conflicts(xe1, xe2, changes, xpp);
               ...
               (xdl_refine_conflicts(xe1, xe2, changes, xpp) < 0 ||
                xdl_simplify_non_conflicts(xe1, changes, XDL_MERGE_ZEALOUS < level) < 0)) {
        xdl_cleanup_merge(changes);
        return -1;
    }
    /* output */
    if (result) {
        ...
        int size = xdl_fill_merge_buffer(xe1, name1, xe2, name2, ancestor_name, favor, changes,
        ...
        result->size = size;
        xdl_fill_merge_buffer(xe1, name1, xe2, name2, ancestor_name, favor, changes, result->ptr,
                              style, marker_size);
    }
    return xdl_cleanup_merge(changes);
}

int xdl_merge(mmfile_t *orig, mmfile_t *mf1, mmfile_t *mf2, xmparam_t const *xmp,
              mmbuffer_t *result)
{
